...
function str2Select(selectStr,column){
    ...
    for (let i=0;i<fields.length;i++){
        ...
        newFields.push(temp===fields[i] ? temp : fields[i]+'='+temp);
    }
...
function getSort(newSort,fixSort){
    let lastSort= newSort || {}; //避免为空
    let keys= [];
    fixSort && Object.keys(fixSort);
    for (let i=0;i<keys.length;i++){
        if (!lastSort.hasOwnProperty(keys[i])){
            lastSort[keys[i]]=fixSort[keys[i]]; //添加排序
        }
    }
    return lastSort;
}
