...
function remove(model,option,cb){
    ...
    Thenjs((cont)=> {
        let WHERE;
        WHERE = sqlHelper.WHERE(option.where||option);
...
function insert(model,option,cb){
    ...
    Thenjs((cont)=>{
        ...
        conn.Execute(sql,values,option.types||types,cont);
    }).then((cont,result)=>{
        if (option.commit){
            conn.Commit((err)=>{
                if (err) return cont(err);
...
function update(model,option,cb){
    ...
    Thenjs((cont)=>{
        ...
        let WHERE=sqlHelper.WHERE(option.where);
        let sql;
        ...
        if (nrows===0 && option.upsert) {
            ...
            VALUES (${_.fill(Array(fields.length), '?').join(',')}) `;
