...
class Model {
    /**
     *
     * @param option {{table:String,[fields]:{[name]:String|[String,String,Number]},[where]:String,[keySort]:Object,[conn]:MyConnection,[connection]:MyConnection}}
     */
    constructor(option) {
        //this.option = option;
        /**
        ...
        if (!this.connection) throw new Error("miss arguments : connection");
        this.conn = this.connection;
        this.sqlHelper = this.conn.pool.sqlHelper;
     ...
     */
    getWhere(option){
        return this.sqlHelper.WHERE(option,this.column.bind(this));
    }
}
...
async function remove(model,option) {
    ...
    try {
        ...
        let sql = `delete from ${model.table} ` + (WHERE ? 'WHERE ' + WHERE : '');
...
async function insert(model,option) {
    ...
    let callbackResult = [];
    this.sqlHelper.INSERT(define, (err, sql, values, types) => {
...
async function update(model,option){
    ...
    let values=[];
        ...
        for (let i=0;i<keys.length;i++){
            ...
            sets.push(keys[i]+'='+this.sqlHelper.VALUE(set[keys[i]],this.column.bind(this)))
        }
