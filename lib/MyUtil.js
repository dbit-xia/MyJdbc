...
class MyUtil {
    ...
    static str_rmInvalidchars(text) {
        ...
        return text.replace(/[^0-9a-zA-Z０-９ａ-ｚＡ-Ｚ\u4e00-\u9fa5\`\~\!\@\#\$\%\^\&\*\(\)\_\+\-\=\{\}\|\:\"\<\>\?\[\]\\\;\'\,\.\/]*/g, '');
    }
     ...
     * @param cb
     */
    static async eachLimit(array,iterator,limit,cb){
        let fns=[];
     ...
     * @param cb {*}
     */
    static async restPages(fn, fixData, oField, iteratee, cb) {
    ...
    static async createDomain(oldCallback,cb) {
