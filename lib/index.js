...
class MyPool extends events{
    ...
    open(cb) {
        ...
        let opts = {
            ...
            //numTestsPerRun:3,
            acquireTimeoutMillis: _config.acquireTimeoutMillis || 3000, //获取连接,20秒超时
...
async function queryLimit(conn,baseSql,option) {
    ...
    async function selectlastSQL() {
        ...
        //首页
        if (startRow <= 1) {
            sql = `SELECT ${distinct?'DISTINCT ':''}${fetchRow ? 'TOP ' + String(fetchRow) : ''} ${SELECT} ${into ? ("into " + into) : ''} FROM ${option.from || baseSql.from} ${where.length ? ('WHERE ' + where.join(' AND ')) : ''}`;
            if (SORT) sql += ` ORDER BY ${SORT}`;
            sql += appendSql;
            if (toSql) return sql; //仅返回SQL
        ...
        sql += `\nSELECT ${distinct?'DISTINCT ':''}TOP ${top} ${selectFieldStr} FROM ${option.from || baseSql.from} ${where.length ? ('WHERE ' + where.join(' AND ')) : ''}`;
        if (SORT) sql += ` ORDER BY ${SORT}`;
        sql += appendSql;
        ...
        sql += `\nSELECT ${distinct?'DISTINCT ':''}TOP ${fetchRow} ${SELECT} ${into ? ("into "+into):''} FROM ${option.from || baseSql.from} ${where.length ? ('WHERE ' + where.join(' AND ')) : ''}`;
