...
class MyPool{
    ...
    async open(cb) {
        ...
        pool.on('factoryCreateError',function (err) {
            ...
            err = _this["group"] + ':' + _this["lastAccessor"] + '\n' + MyUtil.String(err);
    ...
    async clear(){
        this.logger.warn('clear pool begin...');
        await this.pool.drain();
        await this.pool.clear();
        this.logger.warn('clear pool end...');
    }
}
...
class MyConnection extends events{
    ...
    async Commit(cb) {
        ...
        // await checkConnection(this);
        const group = _.get(this, 'pool.group');
        if (!this.connection) return Promise.reject(new Error(group + ':Connection is closed!')); //已被close
        try {
            let result = await this.connection.commit();
...
async function setParams(statement,rows,datatypes,option) {
    ...
    if ((rows instanceof Array) === false) return Promise.reject(new Error('第二个参数必须是数组!'));
    ...
    return await java.callStaticMethod('node.jdbc', 'setParams', statement, JSON.stringify(rows), nTypes, option.isSelect);
...
const QueryBase=async function (conn,option) { /*paramsList SQL参数列表*/
    ...
    try{
        ...
        if (paramsList) { /*有参数*/
            ...
            console.debug(conn.session.group, conn.spid + ':', paramsList, datatypes || '');
 ...
 * @param option {{select:string,where:object,sort:object,limit:[Array],[isolation],grid,into,from,[toSql]}}
 * @constructor
 */
async function QueryLimit(conn,baseSql,option) {
    //console.warn('QueryLimit:',baseSql.keySort,option.sort);
    let {isolation,into,toSql}=option;
    let appendSql = '';
...
function showLog(conn,text,a) {
    conn.logger.info(conn.spid + ':', text, a || ''); //group+':'+
}
