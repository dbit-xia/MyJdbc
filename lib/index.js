                ...
                return new Promise(function (resolve, reject) {
                    ...
                    MyJdbc.createMyConnection(_config, (err, conn) => {
                        ...
                        Thenjs((cont)=>{
                            ...
                            showSql(conn, `spid:${conn.spid},getdate:${conn.getdate()}`);
                            conn.setOption((err) => {
                                if (err) { //set失败则关闭连接
                            ...
                            showSql(conn,'getConnection:'+((new Date().getTime()) - start_create)+' ms');
                            resolve(conn);
                        }).fail((c,err)=>{
                            reject(err);
                        });
...
class MyConnection {
    ...
    Execute(sql, paramsList, datatypes, cb) {
        ...
        return Execute.apply(this, [this].concat(args, (err, result) => {
            if (isCallback === true) {
                let msg = '超时已被关闭:' + sql + ' ' + MyUtil.String(_.get(_this, 'session.souceUrl'));
                return _this.logger.error(_this.spid + ':', msg);
            }
            clearTimeout(timeoutHandle); //清除超时检测
            isCallback = true;
            cb(err, result);
        }));
    }
    ...
    getDBErr(err, cb) {
        ...
        if (_.get(err, 'cause.getErrorCode')) {
            ...
            Thenjs.each(['getErrorCode', 'getMessage', 'getSQLState'], (cont, name) => {
                ...
                if (_this.pool.adapter.getCanIgnoreError().includes(shortError["ErrorCode"])) return cb(null,0);
...
const Query=function (conn,sql,paramsList,datatypes,cb) { /*paramsList SQL参数列表*/
    let args = _.toArray(arguments);
    cb = args.pop();
    paramsList=args[2];
    datatypes=args[3];
    ...
    Thenjs((cont)=> {
        ...
        if (!_.size(paramsList)){
            return statement._ps.executeUpdate((err,result)=>{
                if (err) return cont(err);
                showSql(conn,'executeUpdate:'+result);
                cont(null,result);
            }); //executeUpdate
        }
        showSql(conn,'executeStart:'+_.size(paramsList));
        //分批执行
        ...
        for (let i=0;i<Math.ceil(_.size(paramsList) / batchCount);i++){
            fns.push((cont)=>{
                ...
                Thenjs((cont)=>{
                    ...
                    showSql(conn,'executeBatch:'+rowcount);
                    cont(null,rowcount);
                }).fail((c,err)=>{
                    showSql(conn,data);
                    cont(err);
        ...
        showSql(conn,'Execute rowcount:'+' '+ret);
        cb(null, ret);
    });
}
 ...
 * @constructor
 */
const ExecuteBatch=function (conn,sql,option,cb){
    let args = _.toArray(arguments);
    cb = args.pop();
    option=args[2];
    let {emit}=option || {};
