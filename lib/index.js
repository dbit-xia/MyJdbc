...
class MyPool{
    ...
    open(config,cb){
        ...
        let opts = {
            ...
            //numTestsPerRun:3,
            acquireTimeoutMillis:30000, //获取连接,30秒超时
...
function Execute(myConn,sql,paramsList,datatypes,cb) /*目前不支持点位符*/ {
    ...
    Thenjs((cont)=> {
        ...
        console.debug(_.size(paramsList));
        //分批执行
        ...
        for (let i=0;i<Math.ceil(_.size(paramsList) / batchCount);i++){
            fns.push((cont)=>{
                Thenjs((cont)=>{
                    ...
                    console.debug(i * batchCount,(i + 1)  * batchCount,_.size(data));
        ...
        console.debug('Execute rowcount:',ret);
        cb(null, ret);
    });
}
        ...
        for (let i=0;i<sqlcount;i++){
            series.push((cont)=>{
                ...
                _statement.executeUpdate(sql[i],(err,result)=>{
                    ...
                    console.debug('ExecuteBatch rowcount:',result);
                    cont(null,result);
                });
            });
        }
...
function QueryLimit(myConn,baseSql,option,cb){
    ...
    if (option.where && _.isObject(option.where)===false){
        return cb(new Error('where需要传入对象!'));
    }
