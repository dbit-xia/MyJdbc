'use strict';

const MyUtil=require('../../MyUtil');
const _=require('lodash');
class index {
    constructor() {

    }

    /**
     *
     * @return {string}
     */
    static getdate() {
        return 'getdate()';
    }

    /**
     *
     * @return {string}
     */
    static spid() {
        return '@@spid';
    }

    /**
     *
     * @param conn
     * @param key
     * @return {Promise<*>}
     */
    static async getInfo(conn,key) {
        let rows;
        switch (key) {
            case 'dbname':
                rows = await conn.QueryGrid('select DB_NAME()');
                break;
            case 'spid':
                rows = await conn.QueryGrid('select @@spid');
                break;
            case 'current_time':
                rows = await conn.QueryGrid('select getdate()');
                break;
            default:
                rows = [[null]];
        }
        return rows[0][0];
    }
    /**
     *
     * @param conn {MyConnection}
     */
    static async setOption(conn) {
        // Thenjs((cont) => {
        let rows = await conn.Query("SELECT DB_NAME() as dbname");
        // }).then((cont, rows) => {
        if (_.get(rows, '0.dbname') === 'master') return Promise.reject('DatabaseName not connect!');
        let sqls = [
            "set forceplan on",
            "set plan optgoal allrows_oltp",
            "set arithabort numeric_truncation off",
            "set table count 100",
            "set quoted_identifier off"
        ]; //,"set arithabort arith_overflow off"
        let initialCmds = _.get(conn, 'pool._config.initialCmds');
        if (initialCmds) sqls.push.apply(sqls, initialCmds);

        let fns = [];
        fns.push(conn.ExecuteDDL(sqls.join(' ')));
        return await Promise.all(fns);
    }

    /**
     *
     * @param isolation {number|string}
     * @return {string}
     */
    static appendIsolation(isolation){
        return ' at isolation ' + isolation;
    }

    /**
     *
     * @return {string}
     */
    static getSpidSql(){
        return 'select @@spid as spid,getdate() as getdate';
    }

    /**
     *
     * @return {[]}
     */
    static getCanIgnoreError() {
        /**
         * 207 查询时列不存在
         * 208 select/insert/update/delete 时表不存在
         * 12822 //创建临时表时,表已存在
         * 2714, //创建永久表时,表已存在
         * 1911 列未修改/索引列不存在(1911)
         */
        return [
            1913, //索引已存在1913
            2705, //增加列已存在
            3701, //删除索引时表不存在,删除表不存在,删除的触发器不存在
            13925, //列未修改(?)
            13900,
            13905,
            4963, //锁类型
            1906, //创建索引从属表不存在
            // 17780, //字段重命名时新老字段都存在
            17782, //字段无需重复rename,旧字段不存在
            4902, //修改字段时表不存在
            18228 //修改字段名时表不存在
        ];
    }

    /**
     * @param conn {MyConnection}
     * @param sqlError {{ErrorCode,Message,SQLState,ErrorLevel,appCode}}
     * @return {*}
     */
    static async getSimpleError(conn,sqlError){
        if (!sqlError) return sqlError;
        // sqlError.ErrorCode=2714;
        if (sqlError.ErrorCode===0 && sqlError.Message.includes('Attempt to insert duplicate key row in object')) sqlError.ErrorCode = 2601;
        if (sqlError.ErrorCode===0 && sqlError.Message.includes('Incorrect syntax near the keyword')) sqlError.ErrorCode = 102;
        switch (sqlError.ErrorCode) {
            case 1205: {
                let locktype = MyUtil.posValue(sqlError.Message, "encountered a ", " situation");
                if (locktype === 'deadlock') sqlError.Message = '系统忙(资源处理冲突),稍后重试.';
                break;
            }
            case 2601: { //[Error Code: 2601, SQL State: 23000]  Attempt to insert duplicate key row in object 'db' with unique index 'dbno1_index_db'
                let table = MyUtil.posValue(sqlError.Message, "in object '", "'");
                let index = MyUtil.posValue(sqlError.Message, "unique index '", "'");
                if (index && table) sqlError.Message = '不允许有重复行,约束:' + table + '.' + index;
                sqlError.appCode = "KEY_DUPLICATE";
                break;
            }
            // case 3701:{ //这里走不到,getCanIgnoreError会直接返回成功
            //     //执行DDL时,删除临时表或对象已存在,如:[Error Code: 2714, SQL State: ZZZZZ]  Cannot drop the table '#gt_online_trades', because it doesn't exist in the system catalogs.
            //     let table = MyUtil.posValue(sqlError.Message, "drop the ", ",");
            //     if (table) sqlError.Message = '尝试删除不存在的表或对象:' + table;
            //     sqlError.ErrorLevel = 'WARN';
            //     sqlError.appCode = "OBJECT_NOT_EXISTS";
            //     break;
            // }
            case 2714: {
                //执行DDL时,表或对象已存在,如:[Error Code: 2714, SQL State: ZZZZZ]  There is already an object named 'inctable' in the database.
                let table = MyUtil.posValue(sqlError.Message, "named '", "'");
                if (table) sqlError.Message = '已经存在的表或对象:' + table;
                if (conn.lastSQL && table.startsWith('#')) { //数据库异常,临时表已存在应是:12822
                    try {
                        await conn.Close();
                    } catch (err) {
                        conn.logger.error(err);
                    }
                } else {
                    sqlError.ErrorLevel = 'WARN';
                }
                sqlError.appCode = "OBJECT_EXISTS";
                break;
            }
            case 233: {// [Error Code: 233, SQL State: 23000]  The column dbno in table db does not allow null values.
                let column = MyUtil.posValue(sqlError.Message, " column ", " in ");
                let table = MyUtil.posValue(sqlError.Message, " in table ", " does ");
                if (column && table) sqlError.Message = '不允许为空的字段:' + table + '.' + column;
                break;
            }
            case 257: {// [Error Code: 257, SQL State: 42000]  Implicit conversion from datatype 'INT' to 'VARCHAR' is not allowed.  Use the CONVERT function to run this query.
                let datatype = MyUtil.posValue(sqlError.Message, " to '", "' is ");
                let insertType = MyUtil.posValue(sqlError.Message, " datatype '", "' to ");
                if (insertType && datatype) sqlError.Message = '不允许向' + datatype + '类型的列中写入' + insertType + '类型的数据.'; //存在数据类型不匹配的记录,
                break;
            }
            case 208: {
                //执行DML时,表不存在,如:[Error Code: 208, SQL State: 42000]  db20 not found. Specify owner.objectname or use sp_help to check whether the object exists (sp_help may produce lots of output).
                //执行增加字段时表不存在
                let table = MyUtil.posValue(sqlError.Message, "", " not found");
                if (table) sqlError.Message = '不存在的表或对象:' + table;
                break;
            }
            case 207: {  //[Error Code: 207, SQL State: ZZZZZ]  Invalid column name 'aaaa'.
                let column = MyUtil.posValue(sqlError.Message, "column name '", "'");
                if (column) sqlError.Message = '不存在的列名:' + column;
                if (String(conn.lastSQL).match(/^\s*alter\s+table\s+/i)) { //删除列时列不存在
                    sqlError.ErrorLevel = 'WARN';
                }
                sqlError.appCode = "COLUMN_NOT_EXISTS";
                break;
            }
            case 102: {  //Incorrect syntax near ','. //Incorrect syntax near the keyword \'key\'.\n'
                let words = MyUtil.posValue(sqlError.Message, "'", "'");
                if (words) sqlError.Message = '在 ' + words + ' 附近存在语法错误';
                break;
            }
            case 249: { //Syntax error during explicit conversion of VARCHAR value \\'20\\' to a DATETIME field.\\n
                let sourceType = MyUtil.posValue(sqlError.Message, " of ", " value ");
                let value;
                if (sourceType === 'VARCHAR') {
                    value = MyUtil.posValue(sqlError.Message, " value '", "' to ");
                } else {
                    value = MyUtil.posValue(sqlError.Message, " value ", " to ");
                }

                let destType = MyUtil.posValue(sqlError.Message, " to a ", " field.");
                if (sourceType) sqlError.Message = '' + sourceType + '->' + destType + '数据转换失败:' + value;
                break;
            }
            case 529: { //Syntax error during explicit conversion of VARCHAR value \\'20\\' to a DATETIME field.\\n
                let sourceType = MyUtil.posValue(sqlError.Message, " datatype '", "' to ");
                let destType = MyUtil.posValue(sqlError.Message, " to '", "' is not allowed.");
                if (sourceType) sqlError.Message = '不允许从' + sourceType + '转换为' + destType;
                break;
            }
            case 0:
                switch (sqlError.SQLState) {
                    case 'JZ0BE': {
                        if (sqlError.Message.includes('Some character(s) could not be converted.')) {
                            sqlError.Message = '数据中存在无法支持的特殊字符,请去除后重试!';
                        } else if (sqlError.Message.includes('Arithmetic overflow during implicit conversion of BIGDATETIME value ')) {
                            let value = MyUtil.posValue(sqlError.Message, " value '", "' to ");
                            sqlError.Message = '日期值溢出:' + value;
                        }
                        break;
                    }
                    case 'JZ009': { //JZ009: Error encountered in conversion.  Error message: Timestamp format must be yyyy-mm-dd hh:mm:ss[.fffffffff]
                        if (sqlError.Message.includes('Error encountered in conversion.  Error message: Timestamp format ')) {
                            let format = MyUtil.posValue(sqlError.Message, " format must be ", "");
                            sqlError.Message = '日期/时间格式必须是:' + format;
                        }
                        break;
                    }
                    default:
                }
        }
        return sqlError;
    }
}

exports=module.exports=index;
exports.sqlHelper=require('./sqlHelper');
