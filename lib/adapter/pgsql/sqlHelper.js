...
const fns = {
    ...
    isnull(fields, truevalue) {
        return `coalesce(${fields},${truevalue})`;
    }
};
...
function json2Where(where,column){
    ...
    function EQUALS(key,obj){
        if (obj == null) return column(key) + ' is null ';
...
function getSort(newSort,fixSort){
    if (newSort===false) return {};
    return _.defaults({},newSort,fixSort);
 ...
 * @param option {object|{fields:Array,values:array,types:[],data:[Array]||[Object],table:string,[model]}}
 * @param callback
 * @returns {Array}
 */
async function getInsert(option,callback){
    let {table,model}=option || {};
    if (!table) table=_.get(option,'model.table');
    ...
    try {
        ...
        if (data[0] && _.isObject(data[0])===false) data=[data]; //转换为二维数组
        if (_.isArray(data[0])) { //二维数组
            if ((_.isArray(fields) === false) || _.size(fields) === 0) {
            ...
            if (!option.types) option.types = [];

            let sql = `insert into ${table}(${fields}) values(${new Array(fieldCount).fill('?').join(',')}) `;
            let _types = [];
            _.forEach(data[0], (v,index)=> {
                _types[index] = option.types[index] || _.get(model, 'fields.' + fields[index] + '[1]'); //取model定义的数据类型
            }); //取首行的数据类型
            ...
            let allValues=[];
            let varValues=[]; /*变量值key*/
            let varTypes=[];
            // let _types=[];
            let wordsCheck = /^[A-Za-z_]+[A-Za-z0-9_]*$/; //匹配字段名
            _.forEach(values,(v,n)=> {
                ...
                if (wordsCheck.test(v) === false) { //常量或表达式
                    ...
                    allValues.push('?');
                    varValues.push(values[n]);
                    varTypes.push(option.types[n] || _.get(model, 'fields.' + fields[n] + '[1]'));
                    // _types.push(typeof data[0][v]);
                }
            }); //取首行的数据类型

            let sql=`insert into ${table}(${fields}) values(${allValues.join(',')}) `;

            for (let i=0;i<rowCount;i++){
                newData[i]=[];
            }

            _.forEach(varTypes,(v,n)=>{ //列
                ...
                //     }
                // }else {
                //     if (v === 'object') newTypes[n] = 'string'; //避免object数据类型
                    for (let i = 0; i < rowCount; i++) { //行
                        newData[i][n] = data[i][varValues[n]];
                    }
                // }
            });
            callback(null,sql,newData,varTypes);
        }
...
function buildFIFO(option) {
    ...
    sqls.push.apply(sqls,buildUpdate(option.source[0]));
    sqls.push.apply(sqls,buildUpdate(option.source[1]));
    ...
    sqls.push(`SELECT ${exports.SELECT(option.source[0].select, column.bind(null, 't1.')) + ',' + exports.SELECT(option.source[1].select, column.bind(null, 't2.'))}
        ...
        WHERE ${option.source[0].where ? (exports.WHERE(option.source[0].where, column.bind(null, 't1.')) + ' AND ') : ''} ${option.source[1].where ? (exports.WHERE(option.source[1].where, column.bind(null, 't2.')) + ' AND ') : ''} 
             ${_.map(option.join, (row) => {
    ...
    function buildUpdate(source) {
        let whereStr = exports.WHERE(source.where) || '1=1';
        let nbFields = source.nbFields.split(',');
        let groupArray = source.group.split(',');
        let sortKeys = _.keys(source.sort);
        return [
            `UPDATE ${source.table} t1
            SET ${nbFields[1]}=(SELECT SUM(t2.${nbFields[0]}) FROM ${source.table} t2 
            WHERE ${whereStr + ' AND '}
                  ${_.map(groupArray, (a) => 't2.' + a + '=t1.' + a).join(' AND ')}
                 AND ${_.map(sortKeys, (sortKey) => '(t2.' + sortKey + (source.sort[sortKey] === 'desc' ? '>' : '<') + 't1.' + sortKey + ' OR (t2.' + sortKey + '=t1.' + sortKey).join(' AND ') + (')'.repeat(_.size(sortKeys) * 2))})
                WHERE ${whereStr}`
            , `UPDATE ${source.table} t1 set ${nbFields[2]}=${nbFields[1]} - ${nbFields[0]} WHERE ` + whereStr
        ];
    }
}
